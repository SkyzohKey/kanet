	...
	*/
	...
	namespace Kanet {
	    public class WebServers : Object {
	        ...
	        public signal void update_acl(string message);
	        ...
	        */
	        ...
	        private void rest_acls_handler(Soup.Server server, Soup.Message msg, string path,
	            ...
	            case "GET":
	                ...
	                break;
	            case "PUT":
					klog(@"$TAG Acl PUT with data : $((string)msg.request_body.data)");
					update_acl((string)msg.request_body.data);
	                msg.set_status(Soup.KnownStatusCode.OK);
	                break;
	            case "POST":
					klog(@"$TAG Acl POST with data : $((string)msg.request_body.data)");
	                create_acl((string)msg.request_body.data);
	                msg.set_status(Soup.KnownStatusCode.OK);
	                break;
	            case "DELETE" :
					klog(@"$TAG Acl DELETE with id : $id");
	                delete_acl(id);
	                msg.set_status(Soup.KnownStatusCode.OK);
	                break;
	            }
	        ...
	        private void login_shibboleth_handler(Soup.Server server, Soup.Message msg, string path,
	            ...
	            */
	            ...
	            if(login == null || login == "") {
					klog(@"$TAG null or empty login provided by mod_shib : verify requested attribute set as REMOTE_USER");
					msg.set_status(Soup.KnownStatusCode.FORBIDDEN);
					return;
				}
	            try {
	        ...
	        */
	        ...
	        private bool check_session(Soup.Message msg, Soup.ClientContext client, out Kanet.Session session) {
	            ...
	            if(CONF.get_configuration_value("SERVER_MODE") != "PROXY") {
	                ...
	                ip_src = Utils.get_ip(msg.request_headers.get("X-Forwarded-For").split(",")[0]);
	            }
